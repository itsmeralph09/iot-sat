...
loginForm.addEventListener('submit', (event) => {
    ...
    if (fieldsAreValid) {
        ...
        xhr.open('POST', './function/login_action.php', true);
        xhr.onload = function () {
            ...
            if (xhr.status >= 200 && xhr.status < 300) {
                ...
                if (response.success) {
                    ...
                    setTimeout(() => {
                        ...
                        const role = parseInt(response.usertype);
                        if (role === 1) {
                            window.location.href = './admin/dashboard.php'; // Redirect to admin dashboard
                        } else if (role === 2) {
                            window.location.href = './student/dashboard.php'; // Redirect to student page
                        }
                    }, 1000); // Adjust the delay time as needed (in milliseconds)
                }
                 else {
                    // If AJAX request is successful but response indicates error, show SweetAlert error message
                    showWarningMessage2(response.message || 'Invalid email or password. Please try again.');
                }
            } else {
